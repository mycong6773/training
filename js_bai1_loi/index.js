...
const passRegex = /^(?=.*[a-z])(?=.*[A-Z])[a-zA-Z\d]{8,32}$/;
...
const showMessageError = (error, errorMessage) => {
    error.classList.remove("d-none");
    error.textContent = errorMessage;
};

const hideMessageError = (error) => {
...
const validateField = (e, regex, passwordValue, confirmPasswordValue) => {
    const value = e.target.value.trim();
    const error = e.target.nextElementSibling;
    if (!required(value)) {
        showMessageError(error, "Trường này bắt buộc !!!");
        return;
    }
    if (!matchesRegex(value, regex)) {
        showMessageError(error, "Value is not valid");
        return;
    }
    if (!checkPass(passwordValue, confirmPasswordValue)) {
        showMessageError(error, "pass k dúng");
        return;
    }
    hideMessageError(error);
    // if (!hideMessageError(error)) {
...
confirmPasswordInput.addEventListener("change", function (e) {
    validateField(e, passwordValue, confirmPasswordValue);
});
